...
def get_vote_outcome(flat_index, phc_shape, tensor_demo):
    """
    ...
    tensor_demo (Tensor): the demographics of the district, as a Tensor
    ...
    vote_outcome = tf.math.reduce_sum(tensor_demo * tf.cast(demo_vote_pcts, tf.float32))

    return vote_outcome


@tf.function
def expec_votes(phc, tensor_demo, rwm=False):
    """
...
@tf.function
def prob_from_expec(phc, tensor_demo, observed, rwm=False):
    """
    ...
    return tf.math.log(1. - tf.math.sigmoid(
        tf.math.abs(observed - expec_votes(phc, tensor_demo, rwm=rwm))))
